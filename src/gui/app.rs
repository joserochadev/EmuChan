...
impl EmuChanGui {
	pub fn new() -> Self {
		...
		Self {
			...
			event_rx,
			emulator,
			current_state: EmulatorState::Idle,
...
impl eframe::App for EmuChanGui {
	fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
		self.process_emulator_events(ctx);
...
impl EmuChanGui {
	fn process_emulator_events(&mut self, ctx: &egui::Context) {
		while let Ok(event) = self.event_rx.try_recv() {
			match event {
				EmulatorEvent::FrameReady(frame) => {
					self.update_emulator_texture(ctx, frame);
				}

				EmulatorEvent::StateChanged(state) => {
					self.current_state = state;
				}

				EmulatorEvent::RomLoaded { title, .. } => {
					let new_title = if title.trim().is_empty() {
						"EmuChan".to_string()
					} else {
						let clean_title = title.trim_matches(char::from(0)).trim().to_string();
		...
		egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
			egui::menu::bar(ui, |ui| {
				ui.menu_button("File", |ui| {
					if ui.button("📂 Load Rom").clicked() {
						...
						if let Some(path) = rfd::FileDialog::new()
							...
							.pick_file()
						{
							let _ = self.command_tx.send(EmulatorCommand::LoadRom(path));
						}
				...
				ui.menu_button("Emulation", |ui| {
					let pause_text = if self.current_state == EmulatorState::Running {
						"⏸ Pause"
					} else {
						"▶ Resume"
					};
